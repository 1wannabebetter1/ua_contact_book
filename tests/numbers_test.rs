...
mod numbers_test {
    ...
    fn numbers_creation_test() -> anyhow::Result<()> {
        let pair = Grammar::parse(Rule::numbers, "0904643439, +380904643439")?
            .next()
            .ok_or_else(|| anyhow!("No pair"))?;
        let long_pair = Grammar::parse(Rule::numbers, "+380904643439")?
            .next()
            .ok_or_else(|| anyhow!("No pair"))?;
    ...
    fn wrong_separator() -> anyhow::Result<()> {
        let pair = Grammar::parse(Rule::numbers, "+380904643439. 0904643439")?
            .next()
            .ok_or_else(|| anyhow!("No pair"))?;
