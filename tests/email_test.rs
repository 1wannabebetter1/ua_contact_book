...
mod email_test{
    ...
    fn email_creation_test()-> anyhow::Result<()> {
        let pair =  Grammar::parse(Rule::email, "test.user1@examp.le")?.next().ok_or_else(||anyhow!("No pair"))?;
    ...
    #[test]
    fn to_long_user(){
        let pair =  Grammar::parse(Rule::email, "test.usertest.usertest.usertest.usertest.usertest.usertest.usertest.user@ukr.net");
        assert!(pair.is_err());
    }
}