...
mod city_test{
    ...
    fn city_creation_test()-> anyhow::Result<()> {
        let pair =  Grammar::parse(Rule::city, "Біла Церква , 09103")?.next().ok_or_else(||anyhow!("No pair"))?;
    ...
    fn city_name_with_dot()-> anyhow::Result<()> {
        let pair =  Grammar::parse(Rule::city, "Вашингтон Д. С., 20001")?.next().ok_or_else(||anyhow!("No pair"))?;
