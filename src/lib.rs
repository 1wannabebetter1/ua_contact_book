...
pub fn generate_contact() -> anyhow::Result<Vec<String>, >{
    ...
    let pairs = Grammar::parse(Rule::file, &unparsed_file)?.next().ok_or_else(||anyhow!("No pair"))?;
...
pub fn contact_to_record(user_contact: &str)-> anyhow::Result<Record, >{
    let pairs = Grammar::parse(Rule::contact, user_contact)?.next().ok_or_else(||anyhow!("No pair"))?;
    ...
    return Ok(Record{name, city, birth_date, phone_numbers, email});
}
